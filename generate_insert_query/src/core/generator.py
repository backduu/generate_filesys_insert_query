...
def generate_sql(unique_folders, output_path, run_mode):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"-- 원본 소스: {os.path.basename(output_path).replace('.sql', '.txt')}\n")
...
def generate_file_sql(file_records, output_path, run_mode):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("-- fmsFileInfo 테이블 대량 INSERT 스크립트\n")
        f.write(f"-- 생성 모드: {run_mode}\n")
        f.write("BEGIN TRANSACTION;\n\n")
        
        for i, record in enumerate(file_records):
            bus_home, folder_nm, file_nm, file_capa, file_crt_dt = record
    ...
    print(f"\n[완료] 총 {len(file_records)}개의 파일 INSERT 쿼리가 저장되었습니다!")
